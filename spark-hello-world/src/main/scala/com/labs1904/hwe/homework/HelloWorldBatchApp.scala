...
object HelloWorldBatchApp {
  ...
  def main(args: Array[String]): Unit = {
    try {
      ...
      // A: A spark session is the entry point to running a spark application.  We
      //    need to start building the application so Spark can perform work.
      val spark = SparkSession.builder()
        .appName(jobName)
        .config("spark.sql.shuffle.partitions", "3")
        //TODO- What is local[*] doing here?
        // A: Run Spark locally with as many worker threads as logical cores on your machine.
        .master("local[*]")
        //TODO- What does Get or Create do?
        // A: Gets an existing SparkSession or, if there is no existing one, creates a new one based on the options set in this builder.
        .getOrCreate()
